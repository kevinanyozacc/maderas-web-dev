...
import { useGetRegistroReporteByExp } from '@graphql/api/GetRegistroReporte'
...
const EstadoTramiteModal = ({ isOpen, onClose, tipoTramite }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    // console.log(tramite)
    const data = await useGetRegistroReporteByExp(tramite)
    console.log(data.data)
    if (!data.data.getRegistroReporteByExp!) {
      toast({ type: 'warning', title: 'No se encontró el tramite: ' })
    } else {
      toast({ type: 'success', title: 'Se encontró el tramite: ' })
      setEstadoDatos(data?.data?.getRegistroReporteByExp)
      toggle.onOpen()
    }
  }

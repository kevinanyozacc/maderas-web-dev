...
import { useGetResponsableByExp } from '@graphql/api/GetResponsableByExpediente'
...
const EstadoTramiteModal = ({ isOpen, onClose, tipoTramite }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    // console.log(tramite)
    const data = await useGetResponsableByExp(tramite)
    console.log(data.data)
    if (!data.data.getResponsableByExp!) {
      toast({ type: 'warning', title: 'No se encontró el tramite: ' })
    } else {
      toast({ type: 'success', title: 'Se encontró el tramite: ' })
      setEstadoDatos(data?.data?.getResponsableByExp)
      toggle.onOpen()
    }
  }

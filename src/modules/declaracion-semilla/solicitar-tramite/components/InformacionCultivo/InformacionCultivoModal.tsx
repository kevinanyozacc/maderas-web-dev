...
const InformacionCultivoModal = ({ isOpen, onClose, onSubmit, isUpdate, id }: Props) => {
  ...
  const { values, ...form } = useForm({
    initialValues: isUpdate
      ? store.state.informacionCultivos.find(item => item.id === id)!
      : initialState,
  ...
  return (
    <Modal hasOverlay isOpen={isOpen} onClose={clearForm}>
      <div className="flex z-[70] w-full h-full md:h-max max-w-[600px] sm:rounded-xl shadow-lg bg-white dark:bg-slate-800">
        <form
        ...
        >
          <ModalHeader
            closeBtn={clearForm}
            title={`${isUpdate ? 'Actualizar' : 'Agregar'} Informacion del Cultivar`}
          />

...
import { TramiteExperiencia } from '@generated/graphql'

import { IconEdit, IconTrash } from '@icons'
import moment from 'moment'

interface Props {
  data: TramiteExperiencia[]
  onAdd: () => void
    ...
    <div className="min-h-50">
      {data.length !== 0 && (
        <Table className="min-h-50">
          ...
          <tbody>
            {data?.map((item) => (
              <tr key={item.EXPERIENCIA_RELACIONADA_ID} className="dark:bg-transparent dark:text-slate-50 dark:hover:bg-slate-900 dark:border-b-slate-700"
>
                ...
                <td>
                  <div className="flex justify-center gap-x-3">
                    <button
                      type='button'
                      onClick={() => onUpdate(item.EXPERIENCIA_RELACIONADA_ID!)}
                    ...
                    <button
                      type='button'
                      onClick={() => onDelete(item.EXPERIENCIA_RELACIONADA_ID!)}
                      className="btn-icon btn-ghost-primary"
                    >
                      <IconTrash />
                    </button>

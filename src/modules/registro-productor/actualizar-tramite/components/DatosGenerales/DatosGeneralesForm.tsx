...
import { textSolicitante } from '@modules/registro-productor/utils/textContent'
...
const DatosGeneralesForm = ({ next, registroId }: Props) => {
  ...
  const { fetching, SOLICITANTE, updateSolicitante, dataTramiteEstObs } =
  ...
  return (
    ...
    >
      {!!dataTramiteEstObs.data?.getObservacionesByExpediente
        .OBS_DATOS_GENERALES && (
        <WarningAlert
          message={
            dataTramiteEstObs.data?.getObservacionesByExpediente

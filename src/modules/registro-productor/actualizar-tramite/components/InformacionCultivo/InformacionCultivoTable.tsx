...
import { TramiteInfoCultivo } from '@generated/graphql'
import { IconEdit, IconTrash } from '@icons'

interface Props {
  data?: TramiteInfoCultivo[]
  onAdd: () => void
      ...
      {data?.length !== 0 && (
        <Table>
          ...
          <tbody>
            {data &&
              data.map((item) => (
                <tr key={item.INFORMACION_CULTIVO_ID} className="dark:bg-transparent dark:text-slate-50 dark:hover:bg-slate-900 dark:border-b-slate-700"
                >
                  ...
                  <td>
                    <div className="flex justify-center gap-x-3">
                      <button
                        type='button'
                        onClick={() => onUpdate(item.INFORMACION_CULTIVO_ID)}
                      ...
                      <button
                        type='button'
                        onClick={() => onDelete(item.INFORMACION_CULTIVO_ID)}
                        className="btn-icon btn-ghost-primary"
                      >
                        <IconTrash />
                      </button>

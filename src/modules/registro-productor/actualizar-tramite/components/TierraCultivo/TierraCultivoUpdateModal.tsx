...
import { TierraCultivoInput, useGetTierraCultivoByIdQuery } from '@generated/graphql'
...
const TierrasInitialState: TierraCultivoInput = {
  AREA: '',
  DEPARTAMENTO: '',
  DISTRITO: '',
  EXPEDIENTE_ID: 0,
...
const TierraCultivoUpdateModal = ({ isOpen, onClose, onSubmit, idToUpdate }: Props) => {
  ...
  const [data] = useGetTierraCultivoByIdQuery({
    variables: { tierraCultivoId: idToUpdate },
    pause: !idToUpdate,
    requestPolicy: 'network-only'
  })

  useEffect(() => {
    if (data.data?.getTierraCultivoById) {
      const { __typename, FECHA_REGISTRO, ...rest } = data.data?.getTierraCultivoById!
      form.setFields(rest)
    }
  }, [data.fetching])
...
export default TierraCultivoUpdateModal

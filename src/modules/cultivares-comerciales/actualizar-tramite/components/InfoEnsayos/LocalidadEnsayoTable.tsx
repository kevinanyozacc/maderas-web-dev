...
import { LocalidadEnsayo } from '@generated/graphql'
      ...
      {data.length !== 0 && (
        <Table>
          ...
          <tbody>
            {data &&
              data.map((item) => (
                <tr key={item.LOCALIDAD_ENSAYO_ID} className="dark:bg-transparent dark:text-slate-50 dark:hover:bg-slate-900 dark:border-b-slate-700">
                  ...
                  <td>
                    <div className="flex justify-center gap-x-3">
                      <button
                        onClick={() => onUpdate(item.LOCALIDAD_ENSAYO_ID)}
                        type='button'
                      ...
                      <button
                        onClick={() => onDelete(item.LOCALIDAD_ENSAYO_ID)}
                        type='button'
                        className="btn-icon btn-ghost-primary"
                      >
                        <IconTrash />

...
import { datosGeneralesCultComValid } from '@modules/cultivares-comerciales/validation/datosGeneralesValid'
...
const DatosGeneralesForm = ({ next, registroId }: Props) => {
  ...
  const { datos, updateSolicitante, datosObs } =
  ...
  useEffect(() => {
    if (datos.data?.getTramiteByRegistroId?.SOLICITANTE) {
      const {
        __typename,
        NOMBRE_DEPARTAMENTO,
        NOMBRE_PROVINCIA,
        NOMBRE_DISTRITO,
        ...rest
      } = datos.data?.getTramiteByRegistroId?.SOLICITANTE
      form.setFields(rest)
    }
  }, [datos.fetching !== true])
  ...
  return (
    ...
    >
      {!!datosObs.data?.getObservacionesByExpediente.OBS_DATOS_GENERALES && (
        <WarningAlert
          message={
            datosObs.data?.getObservacionesByExpediente.OBS_DATOS_GENERALES
          }
